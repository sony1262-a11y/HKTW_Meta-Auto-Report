...
from scripts.kol_report import INSIGHT_FIELDS, BREAKDOWNS, SP_FOLDER, OUTPUT_FILE, SHEET_NAME, load_fx_rates, load_existing, merge_and_deduplicate, save_to_excel
...
def debug_fetch_market(market, date_start, date_stop, fx_rates, pa, time_increment=1):
    ...
    logger.info(f"[{market}] KOL accounts: {len(accounts)}")
    if not accounts:
    ...
    for acct in accounts:
        try:
            ...
            logger.info(f"[{market}] {acct['name']}: {len(rows)} rows")
            for r in rows:
    ...
    save_excel(pd.DataFrame(all_raw_rows), f"KOL_raw_{market}_{date_start}_{date_stop}.xlsx", "Raw API Response")
    ...
    try:
        ...
        logger.info(f"[{market}] Creative/campaign lookup for {len(unique_ad_ids)} ads...")
        ...
        logger.info(f"[{market}] Pages: {sum(1 for v in page_name_map.values() if v)}/{len(unique_ad_ids)} | "
...
def main():
    ...
    pa = PowerAutomateClient(); fx_rates = load_fx_rates(pa)
    logger.info("=" * 60)
    logger.info("DEBUG KOL FETCH")
    logger.info(f"Markets: {markets_to_run} | {date_start} -> {date_stop} | FX: {fx_rates}")
    logger.info("=" * 60)
    all_summaries = []; all_transformed = []
    for m in markets_to_run:
        s = debug_fetch_market(m, date_start, date_stop, fx_rates, pa, time_increment)
