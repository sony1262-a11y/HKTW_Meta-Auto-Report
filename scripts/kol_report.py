...
def load_fx_rates(pa=None):
    ...
    if not os.path.exists(filepath):
        logger.warning("FX rates file not found — USD conversion skipped")
        return {}
...
def fetch_market(market, date_start, date_stop, fx_rates, pa, time_increment="monthly"):
...
def main():
    ...
    logger.info(f"KOL Report | {markets_to_run} | {date_start} -> {date_stop} | {time_increment}")
    pa       = PowerAutomateClient()
    fx_rates = load_fx_rates()
    new_frames = []
    for m in markets_to_run:
        df_m = fetch_market(m, date_start, date_stop, fx_rates, pa, ti_api)
    ...
    artifact_name = f"KOL_{mkt_str}_{date_start}_{date_stop}_{time_increment}_{timestamp}.xlsx"
